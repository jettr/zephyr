...
static inline void
z_vrfy_ec_host_cmd_periph_read(const struct device *dev,
...
// TODO ensure that userspace has acces to rx_buffer

	Z_OOPS(z_user_from_copy(&local_rx_ctx, rx_ctx, sizeof(*rx_ctx)));

	z_impl_host_cmd_periph_init(dev, &local_rx_ctx);

	Z_OOPS(z_user_to_copy(&local_rx_ctx, rx_ctx, sizeof(*rx_ctx)));
}
#include <syscalls/ec_host_cmd_periph_read_mrsh.c>
